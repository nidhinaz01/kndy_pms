<script lang="ts">
  ...
  // Manpower tab state
  let manpowerData: ProductionEmployee[] = [];
  let isManpowerLoading = false;
  ...
  // Load manpower data for the selected date
  async function loadManpowerData() {
    if (!selectedDate) return;
    
    isManpowerLoading = true;
    try {
      manpowerData = await fetchProductionEmployees('P1S2', selectedDate);
    } catch (error) {
      console.error('Error loading manpower data:', error);
      manpowerData = [];
    } finally {
      isManpowerLoading = false;
    }
  }
  ...
  async function loadWorkOrdersData() {
    ...
    try {
      console.log(`🔍 Loading work orders for P1S2 on date: ${selectedDate}`);
      
      // Get all prdn_dates records for P1S2 stage (both entry and exit)
      ...
      try {
        const woDetailsIds = workOrdersArray.map(wo => wo.prdn_wo_details.id);
        
        if (woDetailsIds.length > 0) {
          const { data: reportingData, error: reportingError } = await supabase
            ...
            `)
            .eq('is_deleted', false)
            .eq('prdn_work_planning.stage_code', 'P1S2')
            .order('from_date', { ascending: true });

          if (!reportingError && reportingData) {
            // Group by wo_details_id and get the earliest date for each
            const actualStartDates = new Map();
            reportingData.forEach(report => {
              const woDetailsId = (report as any).prdn_work_planning.wo_details_id;
              // Only include work orders that are in our current list
              if (woDetailsIds.includes(woDetailsId) && !actualStartDates.has(woDetailsId)) {
                actualStartDates.set(woDetailsId, report.from_date);
              }
            });

            // Assign actual start dates to work orders
            workOrdersArray.forEach(workOrder => {
              const actualStartDate = actualStartDates.get(workOrder.prdn_wo_details.id);
              if (actualStartDate) {
                workOrder.actualStartDate = actualStartDate;
                console.log(`📅 Actual start date for WO ${workOrder.prdn_wo_details.wo_no}: ${actualStartDate}`);
              }
            });
          }
        }
      } catch (error) {
        console.error('Error fetching actual start dates from work reporting:', error);
      }

      // Filter for active work orders
      workOrdersData = workOrdersArray.filter(workOrder => {
        // Show work orders that have entry dates (started or planned to start)
        return workOrder.entryDates.length > 0;
      });

      // Filter out delivered work orders
      const deliveredSalesOrderIds = new Set();
      const allSalesOrderIds = workOrdersArray.map(wo => wo.sales_order_id);
      
      if (allSalesOrderIds.length > 0) {
        const { data: deliveryData } = await supabase
          .from('prdn_dates')
          .select('sales_order_id')
          .in('sales_order_id', allSalesOrderIds)
          .is('stage_code', null)
          .eq('date_type', 'delivery')
          .not('actual_date', 'is', null);
        
        if (deliveryData) {
          deliveryData.forEach(d => deliveredSalesOrderIds.add(d.sales_order_id));
        }
      }

      // Remove delivered work orders
      workOrdersData = workOrdersData.filter(wo => !deliveredSalesOrderIds.has(wo.sales_order_id));

      // Fetch all prdn_dates for all work orders to calculate current stage
      const allWorkOrderDates = new Map<number, any[]>();
      
      if (allSalesOrderIds.length > 0) {
        try {
          const { data: allWorkOrderDatesData, error: allDatesError } = await supabase
            .from('prdn_dates')
            .select('sales_order_id, date_type, stage_code, actual_date')
            .in('sales_order_id', allSalesOrderIds);
          
          if (!allDatesError && allWorkOrderDatesData) {
            // Group dates by sales_order_id
            allWorkOrderDatesData.forEach((dateRecord: any) => {
              const salesOrderId = dateRecord.sales_order_id;
              if (!allWorkOrderDates.has(salesOrderId)) {
                allWorkOrderDates.set(salesOrderId, []);
              }
              allWorkOrderDates.get(salesOrderId)!.push(dateRecord);
            });
          }
        } catch (error) {
          console.error('Error fetching all work order dates for current stage:', error);
        }
      }

      // Calculate working days difference and current stage for each work order
      workOrdersData = await Promise.all(
        workOrdersData.map(async (workOrder) => {
          const plannedStart = workOrder.entryDates?.[0]?.planned_date;
          const actualStart = workOrder.actualStartDate;
          
          if (plannedStart) {
            try {
              const workingDaysDiff = await getDateDifference(plannedStart, actualStart);
              workOrder.workingDaysDiff = workingDaysDiff;
            } catch (error) {
              console.error(`Error calculating working days for WO ${workOrder.prdn_wo_details?.wo_no}:`, error);
              workOrder.workingDaysDiff = 0;
            }
          } else {
            workOrder.workingDaysDiff = 0;
          }
          ...
          const plannedEnd = lastExit?.planned_date;
          const actualEnd = lastExit?.actual_date;
          
          if (plannedEnd) {
            try {
              const endDaysDiff = await getDateDifference(plannedEnd, actualEnd);
              workOrder.endWorkingDaysDiff = endDaysDiff;
            } catch (error) {
              console.error(`Error calculating end days for WO ${workOrder.prdn_wo_details?.wo_no}:`, error);
              workOrder.endWorkingDaysDiff = 0;
            }
          } else {
            workOrder.endWorkingDaysDiff = 0;
          }
          ...
          const allDates = allWorkOrderDates.get(workOrder.sales_order_id) || [];
          const entryDates = allDates.filter((d: any) => d.date_type === 'entry' && d.actual_date);
          const exitDates = allDates.filter((d: any) => d.date_type === 'exit' && d.actual_date);
      ...
      console.log(`📦 Active Work Orders found for P1S2 on ${selectedDate}:`, workOrdersData);
  ...
  async function loadWorksData() {
    ...
    isWorksLoading = true;
    try {
      worksData = await fetchProductionWorks('P1S2', selectedDate);
    } catch (error) {
  ...
  async function loadPlannedWorksData() {
    ...
    try {
      ...
      plannedWorksData = await fetchWorkPlanning('P1S2', dateStr);
      console.log(`📋 Loaded ${plannedWorksData.length} planned works for ${dateStr}`);
        ...
        plannedWorksData.map(async (plannedWork) => {
          ...
          if (otherWorkCode) {
            try {
              ...
              const stageCode = 'P1S2'; // Stage code for this page
              let query = supabase
  ...
  async function handleAttendanceMarked(event: CustomEvent) {
    ...
    try {
      ...
      // Reload manpower data to reflect changes
      await loadManpowerData();
  ...
  async function handleStageReassigned(event: CustomEvent) {
    ...
    try {
      // Import the reassignEmployee function
      const { reassignEmployee } = await import('$lib/api/production');
      
      // Reassign employee in database
  ...
  async function handleWorkOrderEntry() {
    try {
      ...
      // Conditions: stage_code = 'P1S2', date_type = 'entry', actual_date IS NULL
      const { data: waitingWorkOrders, error: waitingError } = await supabase
        ...
        .eq('date_type', 'entry')
        .is('actual_date', null)
        .lte('planned_date', selectedDate)
        .order('planned_date', { ascending: true });

      if (waitingError) {
        console.error('Error fetching work orders waiting for entry:', waitingError);
        alert('Error fetching work orders waiting for entry');
  ...
  async function handleEntryConfirm() {
    ...
    try {
      ...
      entryProgressMessage = 'Fetching work type information...';
      ...
      if (workTypeError || !workType) {
        ...
        alert(`No work type found for model ${woModel}`);
        isEntryModalLoading = false;
        return;
      }

      // Get work flow for this work type
      entryProgressMessage = 'Fetching work flow details...';
      ...
      if (workFlowError || !workFlow || workFlow.length === 0) {
        entryProgressMessage = '';
        alert(`No work flow found for work type ${workType.id}`);
        isEntryModalLoading = false;
        return;
      }
      ...
      entryProgressMessage = 'Preparing work status records...';
      ...
      entryProgressMessage = `Creating work status records (${statusRecords.length} works)...`;
      ...
      if (statusError) {
        ...
        alert('Error creating work status records');
        isEntryModalLoading = false;
        return;
      }

      console.log(`✅ Created ${statusRecords.length} work status records for work order ${selectedWorkOrder.prdn_wo_details.wo_no}`);

      // Update prdn_dates table with actual entry date
      entryProgressMessage = 'Updating entry date...';
      const { error: datesError } = await supabase
        .from('prdn_dates')
        .update({
      ...
      if (datesError) {
        entryProgressMessage = '';
        console.error('Error updating prdn_dates:', datesError);
        alert('Work status created but failed to update entry date');
        isEntryModalLoading = false;
        return;
      }
      ...
      entryProgressMessage = '';
      alert(`Work Order ${selectedWorkOrder.prdn_wo_details.wo_no} entered into ${stageCode} successfully. ${statusRecords.length} works added.`);
  ...
  async function handleWorkOrderExit() {
    try {
      ...
      if (enteredError) {
        ...
        alert('Error fetching entered work orders');
        isExitModalLoading = false;
        return;
      }

      if (!enteredWorkOrders || enteredWorkOrders.length === 0) {
        alert('No work orders have been entered into this stage');
        isExitModalLoading = false;
        return;
      }
      ...
      if (exitedError) {
        ...
        alert('Error fetching exited work orders');
        isExitModalLoading = false;
        return;
      }
      ...
      availableWorkOrdersForExit = Array.from(uniqueWorkOrders.values());
  ...
  function getCurrentStage(workOrder: any): string {
    // Current stage is pre-calculated when loading work orders data
  ...
  async function loadShiftBreakTimes() {
    if (!selectedDate) {
      shiftBreakTimes = [];
      return;
    }
    ...
    try {
      ...
      if (scheduleError || !scheduleData || scheduleData.length === 0) {
        console.log('⚠️ No shift schedule found for date:', selectedDate);
        shiftBreakTimes = [];
        return;
      }
      ...
      if (breakError) {
        ...
        shiftBreakTimes = breakData || [];
        console.log(`🕐 Loaded ${shiftBreakTimes.length} break times for shift ${shiftId}`);
      }
    } catch (error) {
  ...
  function generatePlanExcel() {
    try {
      ...
      const summaryData = [
        [`P1S2 - Production Plan - ${new Date(selectedDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2-digit' })}`],
        [],
        ['Summary'],
      ...
      // Write file
      XLSX.writeFile(wb, `Work_Planning_P1S2_${selectedDate}.xlsx`);
    } catch (error) {
    ...
    try {
      ...
      const pdfContent = `
        <html>
          ...
          <body>
            <h1>P1S2 - Production Plan - ${new Date(selectedDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2-digit' })}</h1>
            <div class="summary">
              <h2>Summary</h2>
  ...
  function generateReportExcel() {
    try {
      ...
      // Create summary sheet
      const summaryData = [
        [`P1S2 - Production Report - ${new Date(selectedDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2-digit' })}`],
        [],
        ['Summary'],
      ...
      // Write file
      XLSX.writeFile(wb, `Work_Reporting_P1S2_${selectedDate}.xlsx`);
    ...
    try {
      ...
      const pdfContent = `
        <html>
          ...
          <body>
            <h1>P1S2 - Production Report - ${new Date(selectedDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2-digit' })}</h1>
            <div class="summary">
              <h2>Summary</h2>
...
<svelte:head>
  <title>P1S2-General Production Management</title>
</svelte:head>
       ...
       <div class="theme-bg-primary rounded-lg shadow border theme-border">
         <div class="p-6 border-b theme-border">
           <div class="flex items-center justify-between mb-4">
             <h2 class="text-xl font-semibold theme-text-primary">📦 Active Work Orders in P1S2</h2>
           ...
           <p class="theme-text-secondary mb-4">
             Work orders active in P1S2 stage for {new Date(selectedDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2-digit' })} - showing pending and in-progress work orders
           </p>
         ...
         {:else if workOrdersData.length === 0}
           <div class="text-center py-12">
             ...
             <p class="theme-text-secondary text-lg">No active work orders in P1S2</p>
             <p class="theme-text-secondary text-sm mt-2">
               Work orders will appear here when they reach P1S2 stage
             </p>
           </div>
         {:else}
          ...
          <div class="flex items-center justify-between mb-4">
            <div>
              ...
              <p class="theme-text-secondary mt-1">
                Planned works for P1S2 stage on {new Date(selectedDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2-digit' })}
              </p>
            </div>
        ...
        <div class="flex items-center justify-between mb-4">
          <div>
            ...
            <p class="text-sm theme-text-secondary mt-1">
              Reported works for P1S2 stage on {new Date(selectedDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2-digit' })}
            </p>
          </div>
          <div class="flex items-center space-x-3">
  ...
  {#if showEntryModal}
    ...
    >
      ...
      >
        <div class="p-6">
          <div class="flex items-center justify-between mb-4">
            <h2 id="entry-modal-title" class="text-xl font-semibold theme-text-primary">Enter Work Order into P1S2</h2>
            <button
              type="button"
  ...
  {#if showExitModal}
    ...
    >
      ...
      >
        <div class="p-6">
          <div class="flex items-center justify-between mb-4">
            <h2 id="exit-modal-title" class="text-xl font-semibold theme-text-primary">Exit Work Order from P1S2</h2>
            <button
              type="button"
