...
/** Normalize work order id for comparison (Supabase may return id as number or string). */
function normalizeWoId(workOrder: any): number {
  const id = workOrder?.id ?? workOrder?.sales_order_id;
  return id != null ? Number(id) : NaN;
}

export function getWorkOrderDates(workOrder: any, productionDates: any[]) {
  const woId = normalizeWoId(workOrder);
  const woDates = productionDates.filter(d => Number(d.sales_order_id) === woId);
  return {
