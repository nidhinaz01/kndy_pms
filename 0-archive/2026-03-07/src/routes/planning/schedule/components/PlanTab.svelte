<script lang="ts">
  ...
  $: workOrderMap = (() => {
    ...
    scheduleData
      .filter(item => item.planned_date && item.stage_code && (item.date_type === 'entry' || item.date_type === 'exit'))
      .forEach(item => {
        ...
        if (dateType === 'entry') {
          stageData.entry = item.planned_date;
        } else if (dateType === 'exit') {
          stageData.exit = item.planned_date;
        }
      });
...
<div class="p-6">
  {#if workOrders.length === 0}
    <div class="text-center py-12">
      <p class="theme-text-secondary">No planned dates found for the selected date range.</p>
    </div>
  {:else}
