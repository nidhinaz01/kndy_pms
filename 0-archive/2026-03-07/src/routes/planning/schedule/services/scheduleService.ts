 ...
 * Paginates to avoid Supabase 1000-row default limit.
 */
export async function loadScheduleData(fromDate: string, toDate: string): Promise<ScheduleData[]> {
  try {
    // Fetch all date records in pages - we'll filter in-memory for accuracy
    const allData: ScheduleData[] = [];
    let offset = 0;
    let hasMore = true;
    ...
    while (hasMore) {
      ...
      const page = data || [];
      allData.push(...(page as unknown as ScheduleData[]));
