...
export async function convertPlanToDraftReport(
  ...
  try {
    ...
    // Get all approved plans for this stage and date (paginated)
    ...
    // Check which plans already have draft reports (paginated)
    const planIds = approvedPlans.map(p => p.id);
    const existingReports: { planning_id: number }[] = [];
    offset = 0;
    hasMore = true;
    while (hasMore) {
      const { data: reportsPage, error: reportsError } = await supabase
