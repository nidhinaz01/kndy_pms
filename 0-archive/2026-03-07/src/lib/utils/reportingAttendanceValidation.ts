...
export async function validateReportingAttendance(
  ...
  try {
    ...
    // Get all employees assigned to this stage and shift (paginated)
    const employees: { emp_id: string; emp_name?: string; skill_short?: string; shift_code?: string }[] = [];
    let offset = 0;
    let hasMore = true;
    while (hasMore) {
    ...
    // Get all employees who have attendance marked in reporting table (paginated)
    const reportedAttendance: { emp_id: string }[] = [];
    offset = 0;
    hasMore = true;
    while (hasMore) {
