<script lang="ts">
  ...
  import { toggleDerivativeWorkActive, deleteDerivativeWork } from '$lib/api/stdWorkTypeDetails';
  ...
  async function handleEditStatus(row: any) {
    ...
    if (confirm(`Are you sure you want to change the status of "${row.derived_sw_code}" from ${currentStatus} to ${newStatus}?`)) {
      const newActive = !row.is_active;
      try {
        await toggleDerivativeWorkActive(row.swtd_id, newActive);
  ...
  async function handleDelete(row: any) {
    if (confirm(`Are you sure you want to delete the derivative work "${row.derived_sw_code}"?`)) {
      try {
        await deleteDerivativeWork(row.swtd_id);
        // Remove from table data
        tableData = tableData.filter(item => item.swtd_id !== row.swtd_id);
        alert('Derivative work deleted successfully.');
      } catch (error) {
        console.error('Error deleting derivative work:', error);
        alert('Failed to delete derivative work. Please try again.');
      }
    }
  }
  ...
  $: filteredData = tableData.filter(row => {
    return (
      ...
      (!filter.base_work_name || (row.std_work_details?.sw_name || '').toLowerCase().includes(filter.base_work_name.toLowerCase())) &&
      (!filter.type_description || row.type_description.toLowerCase().includes(filter.type_description.toLowerCase())) &&
      (!filter.plant_stage || (row.std_work_details?.plant_stage || '').toLowerCase().includes(filter.plant_stage.toLowerCase())) &&
    ...
    return (
      ...
      (row.std_work_details?.sw_name || '').toLowerCase().includes(s) ||
      row.type_description.toLowerCase().includes(s) ||
      (row.std_work_details?.plant_stage || '').toLowerCase().includes(s)
    );
  });
  ...
  $: sortedData = [...filteredData].sort((a, b) => {
    ...
    if (sortColumn === 'base_work_name') {
      aVal = a.std_work_details?.sw_name || '';
      bVal = b.std_work_details?.sw_name || '';
    } else if (sortColumn === 'plant_stage') {
      aVal = a.std_work_details?.plant_stage || '';
      bVal = b.std_work_details?.plant_stage || '';
    }
  ...
  function exportData() {
    const headers = [
      'Plant Stage', 'Derived Code', 'Base Code', 'Base Work Name', 'Type Description', 'Active', 'Created By', 'Created Date'
    ];
    exportToCSV(
      ...
      'derivative_works',
      (row) => [
        row.std_work_details?.plant_stage || '',
        row.derived_sw_code,
        row.sw_code,
        row.std_work_details?.sw_name || '',
...
<div class="flex items-center justify-between space-x-2 p-3 border-b theme-bg-primary rounded-t-xl shadow mt-4">
  <span class="text-lg font-semibold theme-text-primary">Derivative Works</span>
...
{#if showFilters}
  <div class="p-3 border-b theme-bg-primary">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
      ...
      <div>
        <label for="filterDerivedCode" class="block text-sm font-medium theme-text-primary mb-1">Derived Code</label>
        <input id="filterDerivedCode" type="text" bind:value={filter.derived_sw_code} placeholder="Derived Code" class="w-full px-2 py-1 text-sm border theme-border rounded theme-bg-primary theme-text-primary" />
      </div>
      <div>
        <label for="filterBaseCode" class="block text-sm font-medium theme-text-primary mb-1">Base Code</label>
        <input id="filterBaseCode" type="text" bind:value={filter.sw_code} placeholder="Base Code" class="w-full px-2 py-1 text-sm border theme-border rounded theme-bg-primary theme-text-primary" />
      </div>
      <div>
        <label for="filterBaseWorkName" class="block text-sm font-medium theme-text-primary mb-1">Base Work Name</label>
        <input id="filterBaseWorkName" type="text" bind:value={filter.base_work_name} placeholder="Base Work Name" class="w-full px-2 py-1 text-sm border theme-border rounded theme-bg-primary theme-text-primary" />
      </div>
      <div>
  ...
  {#if isTableLoading}
    <div class="flex items-center justify-center p-8">
      <div class="text-center">
        ...
        <p class="theme-text-primary text-sm">Loading derivative works...</p>
      </div>
    </div>
  {:else}
    <table class="min-w-full table-auto border-separate border-spacing-y-1">
      <thead>
        <tr class="theme-bg-secondary theme-text-primary text-sm">
          ...
          <th class="px-4 py-2 text-left cursor-pointer" on:click={() => handleSort('base_work_name')}>
            Base Work Name
            <svg xmlns="http://www.w3.org/2000/svg" class="inline h-4 w-4 ml-1 transition-transform duration-200 {getSortIcon('base_work_name')}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 11l5-5 5 5M7 13l5 5 5-5" />
            </svg>
          </th>
      ...
      <tbody>
        {#each sortedData as row}
          <tr class="hover:theme-bg-tertiary cursor-pointer" on:click={() => onRowSelect(row)}>
            <td class="px-4 py-2 theme-text-primary">{row.std_work_details?.plant_stage || '-'}</td>
            <td class="px-4 py-2 font-medium theme-text-primary">{row.derived_sw_code}</td>
            <td class="px-4 py-2 theme-text-primary">{row.sw_code}</td>
            <td class="px-4 py-2 theme-text-primary">{row.std_work_details?.sw_name || '-'}</td>
            <td class="px-4 py-2 theme-text-primary">{row.type_description}</td>
            <td class="px-4 py-2">
            ...
            <td class="px-4 py-2" on:click|stopPropagation>
              <div class="flex gap-2">
                ...
                <button
                  ...
                  aria-label="Delete derivative work"
                >
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    {#if sortedData.length === 0}
      <div class="text-center p-8">
        <p class="theme-text-secondary">No derivative works found.</p>
      </div>
    {/if}
  {/if}
</div> 