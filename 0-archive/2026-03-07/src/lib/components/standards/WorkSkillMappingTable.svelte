<script lang="ts">
  ...
  import { toggleWorkSkillMappingActive, deleteWorkSkillMapping } from '$lib/api/stdWorkSkillMapping';
  ...
  async function handleEditStatus(row: any) {
    ...
    if (confirm(`Are you sure you want to change the status of this work-skill mapping from ${currentStatus} to ${newStatus}?`)) {
      const newActive = !row.is_active;
      try {
        await toggleWorkSkillMappingActive(row.wsm_id, newActive);
  ...
  async function handleDelete(row: any) {
    if (confirm(`Are you sure you want to delete this work-skill mapping?`)) {
      try {
        await deleteWorkSkillMapping(row.wsm_id);
        // Remove from table data
        tableData = tableData.filter(item => item.wsm_id !== row.wsm_id);
        alert('Work-skill mapping deleted successfully.');
      } catch (error) {
        console.error('Error deleting work-skill mapping:', error);
        alert('Failed to delete work-skill mapping. Please try again.');
      }
    }
  }
  ...
  $: filteredData = tableData.filter(row => {
    return (
      (!filter.plant_stage || (row.std_work_type_details?.std_work_details?.plant_stage || '').toLowerCase().includes(filter.plant_stage.toLowerCase())) &&
      (!filter.derived_sw_code || row.derived_sw_code.toLowerCase().includes(filter.derived_sw_code.toLowerCase())) &&
      (!filter.work_name || (row.std_work_type_details?.std_work_details?.sw_name || '').toLowerCase().includes(filter.work_name.toLowerCase())) &&
      (!filter.type_description || (row.std_work_type_details?.type_description || '').toLowerCase().includes(filter.type_description.toLowerCase())) &&
      (!filter.sc_name || row.sc_name.toLowerCase().includes(filter.sc_name.toLowerCase())) &&
      (!filter.manpower_required || (row.std_skill_combinations?.manpower_required || '').toString().includes(filter.manpower_required)) &&
    ...
    return (
      (row.std_work_type_details?.std_work_details?.plant_stage || '').toLowerCase().includes(s) ||
      row.derived_sw_code.toLowerCase().includes(s) ||
      (row.std_work_type_details?.std_work_details?.sw_name || '').toLowerCase().includes(s) ||
      (row.std_work_type_details?.type_description || '').toLowerCase().includes(s) ||
      row.sc_name.toLowerCase().includes(s) ||
      (row.std_skill_combinations?.manpower_required || '').toString().includes(s)
    );
  });
  ...
  $: sortedData = [...filteredData].sort((a, b) => {
    ...
    if (sortColumn === 'plant_stage') {
      aVal = a.std_work_type_details?.std_work_details?.plant_stage || '';
      bVal = b.std_work_type_details?.std_work_details?.plant_stage || '';
    } else if (sortColumn === 'work_name') {
      aVal = a.std_work_type_details?.std_work_details?.sw_name || '';
      bVal = b.std_work_type_details?.std_work_details?.sw_name || '';
    } else if (sortColumn === 'type_description') {
      aVal = a.std_work_type_details?.type_description || '';
      bVal = b.std_work_type_details?.type_description || '';
    } else if (sortColumn === 'manpower_required') {
      aVal = a.std_skill_combinations?.manpower_required || 0;
      bVal = b.std_skill_combinations?.manpower_required || 0;
    }
  ...
  function exportData() {
    const headers = [
      'Plant Stage', 'Derived Work Code', 'Work Name', 'Type Description', 'Skill Combination', 'Manpower Required', 'Active', 'Created By', 'Created Date'
    ];
    exportToCSV(
      ...
      'work_skill_mappings',
      (row) => [
        row.std_work_type_details?.std_work_details?.plant_stage || '',
        row.derived_sw_code,
        row.std_work_type_details?.std_work_details?.sw_name || '',
        row.std_work_type_details?.type_description || '',
        row.sc_name,
        row.std_skill_combinations?.manpower_required || '',
...
{#if showFilters}
  <div class="p-3 border-b theme-bg-primary">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-4">
      ...
      <div>
        <label for="filterManpowerRequired" class="block text-sm font-medium theme-text-primary mb-1">Manpower Required</label>
        <input id="filterManpowerRequired" type="text" bind:value={filter.manpower_required} placeholder="Manpower Required" class="w-full px-2 py-1 text-sm border theme-border rounded theme-bg-primary theme-text-primary" />
      </div>
      <div>
  ...
  {#if isTableLoading}
    <div class="flex items-center justify-center p-8">
      <div class="text-center">
        ...
        <p class="theme-text-primary text-sm">Loading work-skill mappings...</p>
      </div>
    </div>
  {:else}
      ...
      <tbody>
        {#each sortedData as row}
          <tr class="hover:theme-bg-tertiary cursor-pointer" on:click={() => onRowSelect(row)}>
            <td class="px-4 py-2 theme-text-primary">{row.std_work_type_details?.std_work_details?.plant_stage || '-'}</td>
            <td class="px-4 py-2 font-medium theme-text-primary">{row.derived_sw_code}</td>
            <td class="px-4 py-2 theme-text-primary">{row.std_work_type_details?.std_work_details?.sw_name || '-'}</td>
            <td class="px-4 py-2 theme-text-primary">{row.std_work_type_details?.type_description || '-'}</td>
            <td class="px-4 py-2 theme-text-primary">{row.sc_name}</td>
            <td class="px-4 py-2 theme-text-primary">{row.std_skill_combinations?.manpower_required || '-'}</td>
            <td class="px-4 py-2">
    ...
    {#if sortedData.length === 0}
      <div class="text-center p-8">
        <p class="theme-text-secondary">No work-skill mappings found.</p>
      </div>
    {/if}
  {/if}
</div> 