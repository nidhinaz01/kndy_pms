<script lang="ts">
  ...
  import { toggleShiftBreakActive, deleteShiftBreak } from '$lib/api/hrShiftBreakMaster';
  ...
  async function handleEditStatus(row: any) {
    ...
    if (confirm(`Are you sure you want to change the status of this shift break from ${currentStatus} to ${newStatus}?`)) {
      const newActive = !row.is_active;
      try {
        await toggleShiftBreakActive(row.break_id, newActive);
  ...
  async function handleDelete(row: any) {
    if (confirm(`Are you sure you want to delete this shift break?`)) {
      try {
        await deleteShiftBreak(row.break_id);
        // Remove from table data
        tableData = tableData.filter(item => item.break_id !== row.break_id);
        alert('Shift break deleted successfully.');
      } catch (error) {
        console.error('Error deleting shift break:', error);
        alert('Failed to delete shift break. Please try again.');
      }
    }
  }
  ...
  $: filteredData = tableData.filter(row => {
    return (
      ...
      (!filter.break_name || (row.break_name || '').toLowerCase().includes(filter.break_name.toLowerCase())) &&
      (!filter.break_number || row.break_number.toString().includes(filter.break_number)) &&
    ...
    return (
      ...
      (row.break_name || '').toLowerCase().includes(s) ||
      row.break_number.toString().includes(s) ||
  ...
  $: sortedData = [...filteredData].sort((a, b) => {
    ...
    if (sortColumn === 'shift_name') {
      aVal = a.hr_shift_master?.shift_name || '';
      bVal = b.hr_shift_master?.shift_name || '';
    } else if (sortColumn === 'break_name') {
      aVal = a.break_name || '';
      bVal = b.break_name || '';
    } else if (sortColumn === 'break_number') {
      aVal = a.break_number || 0;
      bVal = b.break_number || 0;
  ...
  function handleExport() {
    const headers = ['Shift Name', 'Break Name', 'Break Number', 'Start Time', 'End Time', 'Active', 'Created Date'];
    
    exportToCSV(sortedData, headers, 'shift_breaks', (row) => [
...
<div class="flex items-center justify-between space-x-2 p-3 border-b theme-bg-primary rounded-t-xl shadow mt-4">
  ...
  <div class="flex items-center gap-2">
    {#if onAddItem}
      <Button variant="primary" on:click={handleAddItem}>Add Break</Button>
    {/if}
...
{#if showFilters}
  <div class="p-3 border-b theme-bg-primary">
    <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
      ...
      <div>
        <label for="filterBreakName" class="block text-sm font-medium theme-text-primary mb-1">Break Name</label>
        <input id="filterBreakName" type="text" bind:value={filter.break_name} placeholder="Break Name" class="w-full px-2 py-1 text-sm border theme-border rounded theme-bg-primary theme-text-primary" />
      </div>
      <div>
        <label for="filterBreakNumber" class="block text-sm font-medium theme-text-primary mb-1">Break Number</label>
        <input id="filterBreakNumber" type="text" bind:value={filter.break_number} placeholder="Break Number" class="w-full px-2 py-1 text-sm border theme-border rounded theme-bg-primary theme-text-primary" />
      </div>
      <div>
  ...
  {#if isTableLoading}
    <div class="flex items-center justify-center p-8">
      <div class="text-center">
        ...
        <p class="theme-text-primary text-sm">Loading shift breaks...</p>
      </div>
    </div>
  {:else}
    <table class="min-w-full table-auto border-separate border-spacing-y-1">
      <thead>
        <tr class="theme-bg-secondary theme-text-primary text-sm">
          ...
          <th class="px-4 py-2 text-left cursor-pointer" on:click={() => handleSort('break_name')}>
            Break Name
            <svg xmlns="http://www.w3.org/2000/svg" class="inline h-4 w-4 ml-1 transition-transform duration-200 {getSortIcon('break_name')}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 11l5-5 5 5M7 13l5 5 5-5" />
            </svg>
          </th>
          <th class="px-4 py-2 text-left cursor-pointer" on:click={() => handleSort('break_number')}>
            Break Number
            <svg xmlns="http://www.w3.org/2000/svg" class="inline h-4 w-4 ml-1 transition-transform duration-200 {getSortIcon('break_number')}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 11l5-5 5 5M7 13l5 5 5-5" />
            </svg>
          </th>
      ...
      <tbody>
        {#each sortedData as row}
          <tr class="hover:theme-bg-tertiary cursor-pointer" on:click={() => onRowSelect(row)}>
            <td class="px-4 py-2 font-medium theme-text-primary">{row.hr_shift_master?.shift_name || '-'}</td>
            <td class="px-4 py-2 theme-text-primary">{row.break_name || '-'}</td>
            <td class="px-4 py-2 theme-text-primary">{row.break_number || '-'}</td>
    ...
    {#if sortedData.length === 0}
      <div class="text-center p-8">
        <p class="theme-text-secondary">No shift breaks found.</p>
      </div>
    {/if}
  {/if}
</div> 