...
export async function getAvailableWorkOrdersForExit(
  ...
  try {
    // Get work orders that have been entered (have actual entry date) but not exited (paginated)
    const enteredWorkOrders: any[] = [];
    let offset = 0;
    let hasMore = true;
    while (hasMore) {
      const { data, error: enteredError } = await supabase
        .from('prdn_dates')
        .select(`
          *,
          prdn_wo_details!inner(
      ...
      const page = data || [];
      enteredWorkOrders.push(...page);
      hasMore = page.length === PAGE_SIZE;
      offset += PAGE_SIZE;
    }

    if (enteredWorkOrders.length === 0) {
      return [];
    }

    // Get work orders that have been exited (have actual exit date) (paginated)
    const exitedWorkOrders: any[] = [];
    offset = 0;
    hasMore = true;
    while (hasMore) {
      const { data, error: exitedError } = await supabase
        .from('prdn_dates')
