  ...
  {:else}
                  ...
                  {#each typedGroup.items as report}
                    <div class="text-xs">
                      ...
                      <div class="text-xs {report.lt_minutes_total > 0 ? 'text-gray-600' : 'theme-text-secondary'}">
                        Std: {formatReportGroupStandardTimeCell(typedGroup.items[0])}
                      </div>
                      <div class="text-xs {report.lt_minutes_total > 0 ? 'text-gray-600' : 'theme-text-secondary'}">
                        Rem: {formatReportGroupRemainingDisplay(typedGroup)}
                      </div>
                    </div>
                  {/each}
                </div>
    ...
    <div class="mt-6 px-6 py-4 theme-bg-secondary border-t theme-border">
      <div class="flex flex-wrap gap-4 text-sm">
        ...
        <div class="theme-text-secondary">
          <span class="font-medium">Total report rows:</span> {reportData.length}
        </div>
        <div class="theme-text-secondary">
        ...
        <div class="theme-text-secondary">
          <span class="font-medium">Total time worked (till date):</span> {formatTime(
            reportData.reduce((sum, r) => sum + (Number(r.hours_worked_till_date) || 0), 0)
          )}
        </div>
        <div class="theme-text-secondary">
          <span class="font-medium">Total hours worked today:</span> {formatTime(
            reportData.reduce((sum, r) => sum + (Number(r.hours_worked_today) || 0), 0)
          )}
        </div>
        <div class="theme-text-secondary">
        ...
        <div class="theme-text-secondary">
          <span class="font-medium">Total lost time:</span> {formatTime(
            reportData.reduce((sum, r) => sum + (Number(r.lt_minutes_total) || 0), 0) / 60
          )}
        </div>
      </div>
    </div>
