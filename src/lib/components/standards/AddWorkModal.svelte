<script lang="ts">
  ...
  $: if (selectedWorkType) {
    fetchMaxSeqNo(selectedWorkType).then(maxSeq => {
      // Start sequence from 101, and if no records exist, use 101
      seqNo = maxSeq === 0 ? 101 : maxSeq + 1;
      workCode = `${selectedWorkType.charAt(0).toUpperCase()}${seqNo.toString().padStart(4, '0')}`;
    });
  } else {
    workCode = '';
    seqNo = 0;
