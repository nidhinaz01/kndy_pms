<script lang="ts">
  ...
  $: canAddTrainee = formData.selectedTrainees.length < 2;
      ...
      <div class="mb-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg border theme-border">
        ...
        <select
          ...
          on:change={(e) => {
            ...
            if (traineeId) {
              ...
              if (trainee) {
                const isAlreadySelected = formData.selectedTrainees.some(t => t.emp_id === traineeId);
        ...
        >
          ...
          {#each availableTrainees as trainee}
            {@const isAlreadySelected = formData.selectedTrainees.some(t => t.emp_id === trainee.emp_id)}
    ...
    {#if formData.selectedTrainees.length > 0}
        ...
        <div class="flex items-start">
          ...
          <div class="flex-1">
            ...
            <p class="text-xs text-yellow-700 dark:text-yellow-300 mt-1">
              {formData.selectedTrainees.length} trainee{formData.selectedTrainees.length > 1 ? 's' : ''} selected
            </p>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          value={formData.traineeDeviationReason}
          on:input={(e) => onTraineeReasonChange((e.target as HTMLTextAreaElement).value)}
        ></textarea>
        {#if formData.selectedTrainees.length > 0 && !formData.traineeDeviationReason.trim()}
